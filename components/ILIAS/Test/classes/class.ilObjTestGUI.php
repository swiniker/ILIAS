...
class ilObjTestGUI extends ilObjectGUI implements ilCtrlBaseClassInterface, ilDesktopItemHandling
{
    ...
    */
    public function executeCommand(): void
    {
        ...
        switch ($next_class) {
            ...
            case 'illocalunitconfigurationgui':
                ...
                $question_gui->object->setObjId($this->getTestObject()->getId());
            ...
            case 'ilassquestionfeedbackeditinggui':
                ...
                $gui = new ilAssQuestionFeedbackEditingGUI($question_gui, $this->ctrl, $this->access, $this->tpl, $this->tabs_gui, $this->lng);
                $this->ctrl->forwardCommand($gui);
                break;
            ...
            case 'ilobjtestgui':
                ...
                if ($cmd === 'testScreen') {
                    ($this->getTestScreenGUIInstance())->$cmd();
                    return;
                }

                $local_cmd = $cmd . 'Object';
                if (!method_exists($this, $local_cmd)) {
                    $local_cmd = self::DEFAULT_CMD . 'Object';
                }
                $this->$local_cmd();
                break;
    ...
    protected function forwardCommandToExpressPageObject(string $cmd): void
    {
        ...
        $q_gui->object->setObjId($this->getTestObject()->getId());
    ...
    protected function forwardCommandToQuestion(string $cmd): void
    {
        ...
        try {
            ...
            if ($this->getTestObject()->getQuestionSetType() === ilObjTest::QUESTION_SET_TYPE_FIXED
                && !in_array($question_gui->object->getId(), $this->getTestObject()->getQuestions())) {
                $this->getTestObject()->insertQuestion($question_gui->object->getId(), true);
            }

            if ($question_gui->getMoveAfterQuestionId() !== null) {
                $this->getTestObject()->moveQuestionAfter($question_gui->object->getId(), $question_gui->getMoveAfterQuestionId());
            ...
            if ($cmd === 'saveReturn') {
                $this->forwardCommandToQuestionPreview(ilAssQuestionPreviewGUI::CMD_SHOW, $question_gui->object->getId());
            }

            if ($cmd === 'save') {
                $this->ctrl->setParameterByClass(ilAssQuestionPreviewGUI::class, 'q_id', $question_gui->object->getId());
                $this->tabs_gui->setBackTarget($this->lng->txt('backtocallingpage'), $this->ctrl->getLinkTargetByClass(ilAssQuestionPreviewGUI::class, ilAssQuestionPreviewGUI::CMD_SHOW));
    ...
    private function questionsTabGatewayObject()
    {
        ...
        $this->ctrl->redirectByClass('ilObjTestGUI', self::DEFAULT_CMD);
    }
    ...
    */
    public function uploadTstObject()
    {
        ...
        $qti_parser = new ilQTIParser($qti_file, ilQTIParser::IL_MO_VERIFY_QTI, 0, []);
        $qti_parser->startParsing();
        $founditems = $qti_parser->getFoundItems();
        ...
        if ($qti_parser->getQuestionSetType() != ilObjTest::QUESTION_SET_TYPE_FIXED) {
    ...
    */
    public function importVerifiedFileObject()
    {
        ...
        $new_obj->setType($this->testrequest->raw("new_type"));
        ...
        $question_parentobj_id = $new_obj->getId();
        if ($this->testrequest->isset('qpl')) {
            $question_parentobj_id = $this->testrequest->int('qpl');
        }

        if (is_file(ilSession::get("tst_import_dir") . '/' . ilSession::get("tst_import_subdir") . "/manifest.xml")) {
            $new_obj->saveToDb();

            ilSession::set('tst_import_idents', $this->testrequest->raw('ident') ?? '');
            ilSession::set('tst_import_qst_parent', $question_parentobj_id);

            $file_name = ilSession::get('tst_import_subdir') . '.zip';
            $full_path = ilSession::get('tst_import_dir') . '/' . $file_name;
            $imp = new ilImport($this->testrequest->getRefId());
            $map = $imp->getMapping();
            $map->addMapping('components/ILIAS/Test', 'tst', 'new_id', (string) $new_obj->getId());
            $imp->importObject($new_obj, $full_path, $file_name, 'tst', 'components/ILIAS/Test', true);
        } else {
            $idents = $this->testrequest->raw('ident') ?? [];
            $qti_parser = new ilQTIParser(
                ilSession::get("tst_import_qti_file"),
                ilQTIParser::IL_MO_PARSE_QTI,
                $question_parentobj_id,
                $idents
            );
            if ($idents === []) {
            ...
            $question_page_parser = new ilQuestionPageParser(
                $new_obj,
                ilSession::get('tst_import_xml_file'),
                ilSession::get('tst_import_subdir')
            );
            $question_page_parser->setQuestionMapping($qti_parser->getImportMapping());
            $question_page_parser->startParsing();

            if (count($idents) === $qti_parser->getNumImportedItems()) {
                // import test results
        ...
        $this->ctrl->redirect(ilObjTestGUI::class);
    }
    ...
    */
    ...
    public function createQuestionObject(): void
    {
        ...
        $question_gui->object->setObjId($this->getObject()->getId());
        $question_gui->setQuestionTabs();
    ...
    */
    ...
    public function showQuestionsObject()
    {
        ...
        $table_gui->setPositionInsertCommandsEnabled(
            $isset
            && is_array(ilSession::get('tst_qst_move_' . $this->getTestObject()->getTestId()))
            && count(ilSession::get('tst_qst_move_' . $this->getTestObject()->getTestId()))
        );

        $table_gui->setQuestionPositioningEnabled(!$has_started_test_runs);
        $table_gui->setQuestionManagingEnabled(!$has_started_test_runs);
        $table_gui->setObligatoryQuestionsHandlingEnabled($this->getTestObject()->areObligationsEnabled());
        $table_gui->setTotalPoints($this->getTestObject()->getFixedQuestionSetTotalPoints());
        $table_gui->init();
        $table_gui->setData($this->getTestObject()->getTestQuestions());
    ...
    */
    ...
    public function printObject()
    {
        ...
        $template->setVariable("TITLE", ilLegacyFormElementsUtil::prepareFormOutput($this->getTestObject()->getTitle()));
    ...
    */
    ...
    public function copyAndLinkQuestionsToPoolObject(): void
    {
    ...
    public function copyQuestionObject()
    {
        ...
        foreach ($ids as $id) {
            ...
            if ($question) {
                ...
                $clone->object->setObjId($this->getTestObject()->getId());
